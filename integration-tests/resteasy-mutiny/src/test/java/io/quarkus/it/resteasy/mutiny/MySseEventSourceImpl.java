...
public class MySseEventSourceImpl implements SseEventSource {
    ...
    public static class SourceBuilder extends Builder {
        ...
        public SseEventSource build() {
            return new MySseEventSourceImpl(target, name, reconnect, false, executor);
        }
    ...
    public MySseEventSourceImpl(final WebTarget target) {
        this(target, true);
    }

    public MySseEventSourceImpl(final WebTarget target, final boolean open) {
        this(target, null, RECONNECT_DEFAULT, open, null);
    }

    private MySseEventSourceImpl(final WebTarget target, final String name, final long reconnectDelay, final boolean open,
            final ScheduledExecutorService executor) {
        if (target == null) {
            throw new IllegalArgumentException("webTargetIsNotSetForEventSource");
        }
        this.target = target;
    ...
    public void open(String lastEventId, String verb, Entity<?> entity, MediaType... mediaTypes) {
        if (!state.compareAndSet(State.PENDING, State.OPEN)) {
            throw new IllegalStateException("eventSourceIsNotReadyForOpen");
        }
    ...
    private class EventHandler implements Runnable {
        ...
        public void run() {
            ...
            MySseEventInputImpl eventInput = null;
            try {
                ...
                if (Response.Status.Family.SUCCESSFUL.equals(response.getStatusInfo().getFamily())) {
                    ...
                    eventInput = response.readEntity(MySseEventInputImpl.class);
                ...
                if (ex.hasRetryAfter()) {
                    ...
                    System.out.println("MySSE - Reconnecting after Service Unavailable Exception " + ex);
                    reconnect(localReconnectDelay);
                } else {
                    System.out.println("MySSE - unrecoverable ex " + ex);
                    onUnrecoverableError(ex);
                }
                return;
            } catch (Throwable e) {
                System.out.println("MySSE - unrecoverable e " + e);
            ...
            while (!Thread.currentThread().isInterrupted() && state.get() == State.OPEN) {
                if (eventInput == null || eventInput.isClosed()) {
                    System.out.println("MySSE - reconnecting because input is null or closed: " + eventInput);
                ...
                try {
                    ...
                    System.out.println("MySSE - reconnecting because IO Exception: " + e);
                    e.printStackTrace();
                    reconnect(reconnectDelay);
                    break;
                }
