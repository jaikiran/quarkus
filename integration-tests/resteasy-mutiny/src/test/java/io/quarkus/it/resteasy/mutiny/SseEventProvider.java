...
public class SseEventProvider implements AsyncMessageBodyWriter<OutboundSseEvent>, MessageBodyReader<MySseEventInputImpl> {
    ...
    public void writeTo(OutboundSseEvent event, Class<?> type, Type genericType, Annotation[] annotations,
        ...
        if (event.getType() != null) {
            ...
            if (event.getData() != null) {
                ...
                if (writer == null) {
                    throw new ServerErrorException("notFoundMBW " + payloadClass.getName(),
    ...
    public boolean isReadable(Class<?> cls, Type type, Annotation[] annotations, MediaType mediaType) {
        System.out.println(" ====> is readable " + cls + " " + type);
        return MySseEventInputImpl.class.isAssignableFrom(cls) &&
                (MediaType.SERVER_SENT_EVENTS_TYPE.isCompatible(mediaType) ||
                        GENERAL_STREAM_TYPE.isCompatible(mediaType));
    }

    @Override
    public MySseEventInputImpl readFrom(Class<MySseEventInputImpl> cls, Type type, Annotation[] annotations,
        ...
        return new MySseEventInputImpl(annotations, streamType, mediaType, httpHeaders, entityStream);
    }
    ...
    public CompletionStage<Void> asyncWriteTo(OutboundSseEvent event, Class<?> type, Type genericType,
        ...
        if (event.getType() != null) {
            ...
            if (event.getReconnectDelay() > -1) {
                ...
                ret = ret.thenCompose(v -> entityStream
                        .asyncWrite(Long.toString(event.getReconnectDelay()).getBytes(StandardCharsets.UTF_8)));
                ret = ret.thenCompose(v -> entityStream.asyncWrite(SseConstants.EOL));
            }
            ...
            if (event.getData() != null) {
                ...
                AsyncMessageBodyWriter writer = (AsyncMessageBodyWriter) ResteasyProviderFactory.getInstance()
                        .getMessageBodyWriter(payloadClass,
                                payloadType, annotations, event.getMediaType());
                ...
                ret = ret.thenCompose(v -> writer.asyncWriteTo(event.getData(), finalPayloadClass, finalPayloadType,
                        annotations, event.getMediaType(), httpHeaders,
